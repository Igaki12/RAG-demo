document.addEventListener('DOMContentLoaded', () => {
  ...
  const modalRelatedArticles = document.getElementById('modal-related-articles');
  ...
  const modalRelatedArticlesTitle = document.getElementById('modal-related-articles-title');
...
const articles = [
    {
      ...
      questions: [
        {
          question: "ウィンブルドン女子ダブルスで内島萌夏・アンナ・ボンダル組が敗れた相手は誰でしたか？",
          answer: "カテリナ・シニアコバ（チェコ）テーラー・タウンゼント（米国）組"
        },
        {
          question: "男子シングルスでノバク・ジョコビッチがストレートで勝ち上がったのは、何回目でしょうか？",
          answer: "第２回目"
        }
      ]
    },
    {
      ...
      questions: [
        {
          question: "令和２年７月の豪雨で、熊本県球磨村の特別養護老人ホーム「千寿園」で死亡した入所者は何人でしたか？",
          answer: "14人"
        },
        {
          question: "球磨川の河川敷で、住民が石を川に投げ入れるのは、どのような意味がありますか？",
          answer: "災害からくる不幸を願い、復興を祈るため"
        }
      ]
    },
    {
      ...
      questions: [
        {
          question: "６月の月間ＭＶＰ（ア・リーグ）に選ばれたマリナーズの選手は誰ですか？",
          answer: "ローリー"
        },
        {
          question: "６月の月間ＭＶＰ（ナ・リーグ）に選ばれたメッツの選手は誰ですか？",
          answer: "ソト"
        }
      ]
    },
    {
      ...
      questions: [
        {
          question: "佐藤和子さんの年齢はいくつでしたか？",
          answer: "68歳"
        },
        {
          question: "佐藤さんの自宅で見つかった、捜査に役立つ可能性のあるものは何ですか？",
          answer: "ロープ"
        }
      ]
    },
    {
      ...
      questions: [
        {
          question: "米国のウィットコフ特使が核協議を行う予定の国はどこですか？",
          answer: "ノルウェー・オスロ"
        },
        {
          question: "イラン核施設攻撃後、初めての直接協議となるのは、いつになりますか？",
          answer: "来週"
        }
      ]
    },
    {
      ...
      questions: [
        {
          question: "プーチン大統領は、トランプ氏のウクライナでの戦闘終結の呼びかけに対し、どのような姿勢を示しましたか？",
          answer: "侵攻目的を放棄しないと強調した"
        },
        {
          question: "プーチン氏は、和平の条件として、ウクライナに何を求めたのでしょうか？",
          answer: "ＮＡＴＯ加盟断念"
        }
      ]
    },
    {
      ...
      questions: [
        {
          question: "プーチン氏とトランプ氏の電話会談は、最後にいつ行われたのでしょうか？",
          answer: "6月14日"
        },
        {
          question: "トランプ氏が電話会談を行ったのは、就任後何回目でしょうか？",
          answer: "6回目"
        }
      ]
    },
    {
      ...
      questions: [
        {
          question: "シカゴでの銃乱射事件は、何を目当てにパーティーが開かれていましたか？",
          answer: "女性ラッパーのアルバム発表"
        },
        {
          question: "シカゴで過去に起きた銃撃事件と類似点がありますか？",
          answer: "同じ店の前で銃撃事件が起きていた"
        }
      ]
    },
    {
      ...
      questions: [
        {
          question: "米政権が停止した、ウクライナへの支援はどのようなものですか？",
          answer: "兵器供給の一部"
        },
        {
          question: "ウクライナのゼレンスキー大統領は、米国との協議で何を求めていますか？",
          answer: "防空を含む防衛支援"
        }
      ]
    },
    {
      ...
      questions: [
        {
          question: "中国の王毅外相は、レアアースの輸出規制について、どのような姿勢を示しましたか？",
          answer: "規則に基づいた申請があれば、欧州の需要を満たすことができると述べた"
        },
        {
          question: "ワーデフール外相は、中国のレアアース輸出規制について、どのような批判をしましたか？",
          answer: "一方的で透明性に欠ける"
        }
      ]
    }
  ...
  function displayResults(results) {
    ...
    results.forEach((result, index) => {
      ...
      el.className = 'result-item-btn result-item bg-white p-5 rounded-xl shadow-md border border-gray-200 hover:shadow-lg hover:border-blue-400 transition-all duration-200';
    ...
    `;
      el.addEventListener('click', () => showArticleDetail(index));
      resultsContainer.appendChild(el);
    });
  }

  // === UI: Modal ===
  function showArticleDetail(articleIndex) {
    const mainArticle = articles[articleIndex];
    const mainArticleEmbedding = cachedArticleEmbeddings[articleIndex];

    // 他の記事との類似度を計算
    const related = articles
      .map((article, i) => {
        if (i === articleIndex) return null; // 自分自身は除外
        const sim = cosineSimilarity(mainArticleEmbedding, cachedArticleEmbeddings[i]);
        return { ...article, similarity: sim };
      })
      .filter(Boolean) // nullを除外
      .sort((a, b) => b.similarity - a.similarity); // 類似度で降順ソート

    // モーダルに内容をセット
    modalTitle.textContent = mainArticle.title;
    modalContent.textContent = mainArticle.content;
    modalContent.classList.remove('hidden');
    modalArticleTitle.classList.remove('hidden');
    modalRelatedArticles.classList.remove('hidden');
    modalRelatedArticlesTitle.classList.remove('hidden');


    // ニュース内容を確認する時事問題に挑戦
    modalQuizContainer.innerHTML = ''; // Clear previous quiz
    if (mainArticle.questions && mainArticle.questions.length > 0) {
      const quizButton = document.createElement('button');
      quizButton.textContent = '関連問題に挑戦する';
      quizButton.className = 'w-full bg-purple-600 text-white font-bold py-3 px-4 rounded-lg hover:bg-purple-700 focus:outline-none focus:ring-4 focus:ring-purple-300 transition duration-150 ease-in-out';
      
      quizButton.addEventListener('click', () => {
        showQuiz(mainArticle);
      });

      const quizWrapper = document.createElement('div');
      quizWrapper.className = 'mb-6';
      const quizTitle = document.createElement('h3');
      quizTitle.className = 'text-lg font-semibold text-gray-800 mb-4';
      quizTitle.textContent = 'ニュース理解度クイズ';
      quizWrapper.appendChild(quizTitle);
      quizWrapper.appendChild(quizButton);
      modalQuizContainer.appendChild(quizWrapper);
    }


    modalRelatedArticles.innerHTML = '';
    related.slice(0, 5).forEach(article => { // 上位5件を表示
      const el = document.createElement('button');
      el.className = 'related-article-btn p-3 bg-gray-50 rounded-lg border text-left w-full hover:bg-gray-100 hover:border-blue-400 focus:outline-none focus:ring-2 focus:ring-blue-400 transition-all duration-150';
      
      const originalIndex = articles.findIndex(a => a.title === article.title);

      el.innerHTML = `
      <h4 class="font-semibold text-gray-800">${article.title}</h4>
      <p class="text-sm text-gray-600 mt-1">類似度: ${article.similarity.toFixed(3)}</p>
    `;
      el.addEventListener('click', () => {
        if (originalIndex !== -1) {
          closeModal();
          // closeModalのアニメーションが終わるのを待ってから次のモーダルを開く
          setTimeout(() => {
            showArticleDetail(originalIndex);
          }, 300);
        }
      });
      modalRelatedArticles.appendChild(el);
    });
  ...
  function showQuiz(article) {
    modalArticleTitle.classList.add('hidden');
    modalContent.classList.add('hidden');
    modalQuizContainer.innerHTML = ''; // Clear button

    const quizTitle = document.createElement('h3');
    quizTitle.className = 'text-lg font-semibold text-gray-800 mb-4';
    quizTitle.textContent = 'ニュース理解度クイズ';
    modalQuizContainer.appendChild(quizTitle);

    article.questions.forEach((q, index) => {
    const questionEl = document.createElement('div');
    questionEl.className = 'mb-4 p-4 bg-gray-50 rounded-lg opacity-0 transform translate-y-4 transition-all duration-300';
    questionEl.innerHTML = `<p class="font-semibold">${index + 1}. ${q.question}</p>`;
    
    // Add element to DOM first
    modalQuizContainer.appendChild(questionEl);
    ...
    showAnswerButton.textContent = '解答を表示する';
    showAnswerButton.className = 'w-full bg-green-600 text-white font-bold py-3 px-4 rounded-lg hover:bg-green-700 focus:outline-none focus:ring-4 focus:ring-green-300 transition duration-150 ease-in-out';
    showAnswerButton.addEventListener('click', () => {
      showAnswers(article);
    });
    modalQuizContainer.appendChild(showAnswerButton);
  }

  function showAnswers(article) {
    modalArticleTitle.classList.remove('hidden');
    modalContent.classList.remove('hidden');
    modalQuizContainer.innerHTML = ''; // Clear questions and button

    const quizTitle = document.createElement('h3');
    quizTitle.className = 'text-lg font-semibold text-gray-800 mb-4';
    quizTitle.textContent = 'クイズの解答';
    modalQuizContainer.appendChild(quizTitle);

    article.questions.forEach((q, index) => {
    const answerEl = document.createElement('div');
    answerEl.className = 'mb-4 p-4 bg-blue-50 rounded-lg opacity-0 transform translate-y-4 transition-all duration-300';
    answerEl.innerHTML = `
      <p class="font-semibold">${index + 1}. ${q.question}</p>
      <p class="mt-2 text-blue-800"><span class="font-bold">A:</span> ${q.answer}</p>
    `;
    modalQuizContainer.appendChild(answerEl);
    
    // Trigger animation with delay
    setTimeout(() => {
      answerEl.classList.remove('opacity-0', 'translate-y-4');
    }, index * 200);
    });

    const backToArticleButton = document.createElement('button');
    backToArticleButton.textContent = '記事に戻る';
    backToArticleButton.className = 'w-full mt-4 bg-gray-500 text-white font-bold py-3 px-4 rounded-lg hover:bg-gray-600 focus:outline-none focus:ring-4 focus:ring-gray-300 transition duration-150 ease-in-out';
    backToArticleButton.addEventListener('click', () => {
      // Simply re-render the quiz button, hide answers, show article
      modalArticleTitle.classList.remove('hidden');
      modalContent.classList.remove('hidden');
      modalQuizContainer.innerHTML = ''; // Clear answers
    ...
    modalQuizContainer.appendChild(backToArticleButton);
  }
